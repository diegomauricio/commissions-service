...
public class NuevoFijo {
    ...
    public static void main(String[] args) {
        ...
        Origen ori=new Origen("Facturacion manual Corporate", "select * from VENTAS_FIJO where periodo=convert(varchar(6),dateadd(month,-1,getdate()),112)\n" +
        "AND fecha_datos IN\n" +
        "(\n" +
        "	SELECT max(fecha_datos)\n" +
        "	from VENTAS_FIJO where periodo=convert(varchar(6),dateadd(month,-1,getdate()),112)\n" +
        ")", Servidores.SQLSERVER);
        Destino des=new Destino("Facturacion manual Comisiones", "T_VENTAS_FIJO_CORP_SRC", Servidores.ORACLESERVER);
